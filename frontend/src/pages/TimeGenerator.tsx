...
import { Sparkles, Send, Clock, CheckCircle, XCircle, Loader2, ChevronDown, Globe, Zap } from 'lucide-react';
...
export default function TimeGenerator() {
  ...
  const fetchPages = async () => {
    try {
      ...
      setPages(data);
      if (data.length > 0) setSelectedPageIds([data[0].id]);
    } catch { /* silently fail */ }
  };
              ...
              <div className="relative">
                ...
                {showPageDrop && (
                  <div className={`absolute top-full left-0 right-0 mt-1.5 rounded-xl border shadow-xl z-20 overflow-hidden ${isDarkMode ? 'bg-[#0f0f1a] border-white/10' : 'bg-white border-slate-100'}`}>
                    {pages.length === 0 ? (
                      <p className={`px-4 py-3 text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                        No pages connected. Connect a Facebook account first.
                      </p>
                    ) : pages.map(page => (
                      <button
                        key={page.id}
                      ...
                      >
                        <div className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${selectedPageIds.includes(page.id) ? 'bg-blue-500 border-blue-500' : isDarkMode ? 'border-white/20' : 'border-slate-300'}`}>
                    ...
                    <div className="p-2 border-t border-white/5">
                      <button onClick={() => setShowPageDrop(false)} className="w-full py-1.5 text-xs font-bold text-blue-400 hover:text-blue-300">
                        Done
                      </button>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className={`w-full rounded-xl px-3 py-2.5 text-sm outline-none ${input}`}
                />
              </div>
              <div>
          ...
          <div className={`lg:col-span-2 rounded-2xl p-8 flex flex-col items-center justify-center text-center ${card}`}>
            ...
            <div className={`mt-6 grid grid-cols-3 gap-4 w-full max-w-sm text-xs font-bold ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
              {['1. Enter topic', '2. AI writes posts', '3. Auto-scheduled'].map((s, i) => (
                <div key={i} className={`rounded-xl p-3 ${isDarkMode ? 'bg-white/3' : 'bg-slate-50'}`}>
                  {s}
                </div>
              ))}
            </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <h3 className={`text-lg font-black ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                Review Posts ({generatedPosts.length})
              </h3>
              <p className={`text-xs ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                Edit any post before scheduling. All times are peak engagement hours.
              </p>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setStep('setup')}
                className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${isDarkMode ? 'bg-white/5 text-white/50 hover:bg-white/10' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
              >
                ← Regenerate
              </button>
              <button
                ...
                className="flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-black text-white bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 shadow-lg shadow-emerald-500/20 transition-all disabled:opacity-50"
              >
                {isScheduling
                  ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Scheduling {scheduledCount}/{generatedPosts.length}</>
                  : <><Send className="w-3.5 h-3.5" /> Schedule All</>
                }
              </button>
            </div>
          </div>
            ...
            {generatedPosts.map((post, i) => (
              <div key={i} className={`rounded-2xl p-4 space-y-3 border transition-all ${
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {post.status === 'pending' && (
                      <button
                        ...
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-lg transition-all ${isDarkMode ? 'text-red-400/60 hover:bg-red-500/10 hover:text-red-400' : 'text-red-400 hover:bg-red-50'}`}
                      >
                        remove
                      </button>
                    )}
                ...
                <textarea
                  ...
                  rows={4}
                  className={`w-full text-sm rounded-xl px-3 py-2 outline-none resize-none transition-all ${
                    post.status !== 'pending' ? 'opacity-60 cursor-default' : ''
                  } ${isDarkMode ? 'bg-white/5 text-white/80 border border-white/10 focus:border-blue-500/40' : 'bg-slate-50 text-slate-800 border border-slate-100 focus:border-blue-300'}`}
                />
              </div>
            ))}
          </div>
      ...
      {step === 'done' && (
        <div className={`rounded-2xl p-10 flex flex-col items-center justify-center text-center ${card}`}>
          ...
          <p className={`text-sm mb-6 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
            AI-generated posts have been added to your schedule.
          </p>
          <div className="flex items-center gap-3">
            <button
              onClick={() => { setStep('setup'); setGeneratedPosts([]); setTopic(''); }}
              className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${isDarkMode ? 'bg-white/5 text-white/70 hover:bg-white/10' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
            >
              Generate More
            </button>
            <a
              href="/my-posts"
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-black text-white bg-gradient-to-r from-blue-600 to-violet-600 hover:from-blue-500 hover:to-violet-500 shadow-lg shadow-blue-500/20 transition-all"
            >
